...
object GShockPairingManager {
    private val CASIO_SERVICE_UUID: UUID = UUID.fromString("00001804-0000-1000-8000-00805f9b34fb")
    ...
    fun associate(
        ...
        val deviceManager =
                context.getSystemService(Context.COMPANION_DEVICE_SERVICE) as?
        ...
        val deviceFilter =
                BluetoothDeviceFilter.Builder()
                        .setNamePattern(
                                Pattern.compile(DEVICE_NAME_PATTERN, Pattern.CASE_INSENSITIVE)
                        )
                        .build()

        val builder =
                AssociationRequest.Builder().addDeviceFilter(deviceFilter).setSingleDevice(true)
                ...
                object : CompanionDeviceManager.Callback() {
                    ...
                    override fun onFailure(error: CharSequence?) {
                        val errorMessage: String =
                                error?.toString() ?: "Companion device pairing failed"
                        onError(errorMessage)
                    }
                }

        if (AndroidVersion.isTiramisuOrAbove) {
            deviceManager.associate(pairingRequest, context.mainExecutor, callback)
        } else {
            @Suppress("DEPRECATION") deviceManager.associate(pairingRequest, callback, null)
        }
    }
    ...
    fun getAssociationsWithNames(context: Context): List<IGShockAPI.Association> {
        ...
        return if (AndroidVersion.isTiramisuOrAbove) {
            ...
            @Suppress("DEPRECATION") val associations = deviceManager.associations
            associations.map { info -> IGShockAPI.Association(info, null) }
        }
    }
    ...
    fun disassociate(context: Context, address: String) {
        ...
        try {
            if (AndroidVersion.isTiramisuOrAbove) {
                val association =
                        deviceManager.myAssociations.find {
            ...
            @Suppress("DEPRECATION") deviceManager.disassociate(address)
        } catch (e: Exception) {
    ...
    fun startObservingDevicePresence(context: Context, address: String) {
        ...
        if (AndroidVersion.isBaklavaOrAbove) {
            startObservingApi36(deviceManager, address)
        } else {
            @Suppress("DEPRECATION") deviceManager.startObservingDevicePresence(address)
        }
    }

    @RequiresPermission(Manifest.permission.REQUEST_OBSERVE_COMPANION_DEVICE_PRESENCE)
    @RequiresApi(Build.VERSION_CODES.BAKLAVA)
    private fun startObservingApi36(deviceManager: CompanionDeviceManager, address: String) {
        val association =
    ...
    fun stopObservingDevicePresence(context: Context, address: String) {
        ...
        if (AndroidVersion.isBaklavaOrAbove) {
            ...
            val request: ObservingDevicePresenceRequest =
            ...
            @Suppress("DEPRECATION") deviceManager.stopObservingDevicePresence(address)
        }
    }
}
